-- Supply Samples Swap Table
CREATE TABLE IF NOT EXISTS ${forecastDbName}.supply_sample_next_local ON CLUSTER '{cluster}'
(
...
ENGINE = ReplicatedMergeTree('/clickhouse/{cluster}/tables/{shard}/{database}/supply_sample_next_local', '{replica}')
...
CREATE TABLE IF NOT EXISTS ${forecastDbName}.supply_sample_next ON CLUSTER '{cluster}' AS ${forecastDbName}.supply_sample_next_local
ENGINE = Distributed('{cluster}', '{database}', 'supply_sample_next_local', rand());
